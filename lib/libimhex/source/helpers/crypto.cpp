...
namespace hex::crypt {
    ...
    static u8 decodeHexDigit(char value) {
        if (value >= '0' && value <= '9')
            return value - '0';
        if (value >= 'A' && value <= 'F')
            return value - 'A' + 0x0A;
    ...
    std::vector<u8> decode16(const std::string &input) {
        ...
        std::vector<u8> output(input.size() / 2, 0x00);
        for (size_t index = 0; index < input.size(); index += 2) {
            const auto high = decodeHexDigit(input[index]);
            const auto low  = decodeHexDigit(input[index + 1]);
