...
function(initDependencySourceChoice varName name)
    if(NOT DEFINED CACHE{${USE_SYSTEM_${varName}}})
        cmake_parse_arguments("" "PKGCONFIG" "VERSION" "" ${ARGN})
        if(_PKGCONFIG)
            if(_VERSION)
                pkg_check_modules("${varName}" "${name}>=${_VERSION}")
            else()
                pkg_check_modules("${varName}" "${name}")
            endif()
            set(found "${${varName}_FOUND}")
        else()
            if(_VERSION)
                find_package("${name}" "${_VERSION}")
            else()
                find_package("${name}")
            endif()
            set(found "${${name}_FOUND}")
        endif()
        set("USE_SYSTEM_${varName}" "${found}")
        set("USE_SYSTEM_${varName}" "${USE_SYSTEM_${varName}}" CACHE BOOL "Use ${varName} from system-installed packages")
        set("USE_SYSTEM_${varName}" "${USE_SYSTEM_${varName}}" PARENT_SCOPE)
        set("${varName}_DESIRED_VERSION" "${_VERSION}" PARENT_SCOPE)
        set("${varName}_PACKAGE_NAME" "${name}" PARENT_SCOPE)
    endif()
endfunction()
...
macro(addBundledLibraries)
    ...
    #list(APPEND badCloneExclusions "imgui")
    ...
    initDependencySourceChoice(FMT fmt VERSION "8.0.0")
    if(NOT USE_SYSTEM_FMT)
        add_subdirectory(${EXTERN_LIBS_FOLDER}/fmt EXCLUDE_FROM_ALL)
        set_target_properties(fmt PROPERTIES POSITION_INDEPENDENT_CODE ON)
        set(FMT_LIBRARIES fmt::fmt-header-only)
    else()
        find_package(fmt "${FMT_DESIRED_VERSION}" REQUIRED)
        set(FMT_LIBRARIES fmt::fmt)
        list(APPEND badCloneExclusions "fmt")
    endif()
    ...
    initDependencySourceChoice(NLOHMANN_JSON nlohmann_json VERSION "3.10.2")
    if(NOT USE_SYSTEM_NLOHMANN_JSON)
        add_subdirectory(${EXTERN_LIBS_FOLDER}/nlohmann_json EXCLUDE_FROM_ALL)
        set(NLOHMANN_JSON_LIBRARIES nlohmann_json)
    else()
        find_package(nlohmann_json "${NLOHMANN_JSON_DESIRED_VERSION}" REQUIRED)
        set(NLOHMANN_JSON_LIBRARIES nlohmann_json::nlohmann_json)
        list(APPEND badCloneExclusions "nlohmann_json")
    endif()

    initDependencySourceChoice(CURL CURL VERSION "7.76.1")
    if(NOT USE_SYSTEM_CURL)
        ...
        find_package(CURL "${CURL_DESIRED_VERSION}" REQUIRED)
        list(APPEND badCloneExclusions "curl")
    endif()
    ...
    initDependencySourceChoice(YARA yara PKGCONFIG)
    if (NOT USE_SYSTEM_YARA)
        ...
        list(APPEND badCloneExclusions "yara")
    endif()

    initDependencySourceChoice(MINIAUDIO miniaudio PKGCONFIG)
    if (NOT USE_SYSTEM_MINIAUDIO)
        ...
        list(APPEND badCloneExclusions "miniaudio")
    endif()

    initDependencySourceChoice(CAPSTONE capstone PKGCONFIG VERSION "4.0.2")
    if (NOT USE_SYSTEM_CAPSTONE)
        ...
        list(APPEND badCloneExclusions "capstone")
    endif()
    ...
    #list(APPEND badCloneExclusions "pattern_language")
